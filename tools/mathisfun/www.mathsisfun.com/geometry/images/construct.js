var g,g2,el1,el2,my={}
function constructMain(filename,w,h,rel){var version='0.673';if(typeof(rel)==='undefined')rel="../index.html";var w=930;var h=w*9/16;var constQ=true
if(constQ){var w=560;var h=440;}
my.frameTime=100
my.frameLast=Date.now()
console.log('w, h, time',w,h,my.frameTime)
my.voiceQ=false
...
s+='<div id="clicker" style="position: absolute; width:'+w+'px; height:'+h+'px; left: 0; top:; border: none; ; z-index:6;"></div>';s+='<div style="font: 11px Arial; color: #6600cc; position:absolute; right:6px; bottom:3px;">&copy; 2019 MathsIsFun.com  v'+version+'</div>';s+='<div id="filename" style="font: 10px Arial; font-weight: bold; color: #aaa; position:absolute; right:6px; top:3px;"></div>';s+='</div>';document.write(s);el1=document.getElementById('canvas1');my.ratio=2;el1.width=w*my.ratio;el1.height=h*my.ratio;el1.style.width=w+"px";el1.style.height=h+"px";el2=document.getElementById('canvas2');el2.width=w*my.ratio;el2.height=h*my.ratio;el2.style.width=w+"px";el2.style.height=h+"px";if(my.voiceQ){voiceSetup()}else{my.voice=new Voice()}
var el=el1;g=el.getContext("2d")
...
var div=document.getElementById('show'+my.showNo);div.style.opacity=0;my.showNo=1-my.showNo;var div=document.getElementById('show'+my.showNo);div.style.opacity=1;div.style.top='0px';div.innerHTML=fmtShow(my.showTxt);my.scrollYTgt=0;}},add:{n:0,run(cmd){console.log("add");my.showTxt+=cmd.txt
var div=document.getElementById('show'+my.showNo);div.style.opacity=0;my.showNo=1-my.showNo;var div=document.getElementById('show'+my.showNo);div.innerHTML=fmtShow(my.showTxt);div.style.opacity=1;}},say:{n:0,run(cmd){if(my.fastQ)return
if(!my.voiceQ)return
console.log("SAY",cmd.txt);var utterance=new SpeechSynthesisUtterance(cmd.txt);utterance.rate=0.9;utterance.pitch=1.1;utterance.voice=my.voice.getVoices()[2];my.voice.speak(utterance);}},preset:{n:0,run(cmd){var preName=cmd.txt
console.log(">>",cmd.type,preName);var preNo=-1;for(var i=0;i<my.presets.length;i++){var preset=my.presets[i];console.log(">>>",i,preName,preset,preset[0].toLowerCase());if(preset[0].toLowerCase()==preName){preNo=i;break;}}
console.log(">>>",preNo);if(preNo>=0){var preset=my.presets[preNo][1];var bits=preset.split(' ');var cmd=bits[0];var data=bits.splice(1);console.log(">>",cmd.type,cmd,data);cmdDo(new Cmd(cmd,data));}}},align:{n:1,run(cmd){switch(cmd.parms[0].toLowerCase()){case 'c':case 'center':my.showAlign='center'
break
case 'l':case 'left':my.showAlign='left'
break
case 'r':case 'right':my.showAlign='right'
break
default:}}},img:{n:1,run(cmd){var txt='<div style="text-align:center; height: 80%; margin-top: 5%; display: inline-block;"><img style="height:100%;" src="'+cmd.txt+'" /></div>';console.log("IMG",txt);var div=document.getElementById('image');div.innerHTML=txt;}},scroll:{n:0,run(cmd){my.scrollXTgt=parseFloat(cmd.parms[0]);my.scrollYTgt=parseFloat(cmd.parms[1]);console.log(">>>>>>>>>>scroll",my);}},bg:{n:0,run(cmd){console.log("bg",cmd.txt);var div=document.getElementById('bg');div.style.background=cmd.txt}},line:{n:4,run(cmd){var p=cmd.parms
...
my.cmds=[];my.cmdStt=0;my.cmdEnd=0;my.currLvl=0;my.prevX=0;my.prevY=0;my.playQ=false;my.strokedQ=true;loadChars();filename=my.imgHome+'construct/'+filename+'.txt'
getFileFromServer(filename);my.waitUntil=performance.now();}
...
class Voice{constructor(){this.speaking=false
this.pending=false}
speak(){return ''}
cancel(){return ''}}
function getFileFromServer(url){console.log('getFileFromServer',url)
...
line=line.replace(/\s*\/\/.*/,"");if(line.length==0)continue
s+=line+'\n'}
...
my.cmds=cmds
my.cmdEnd=my.cmds.length;my.notes=[];animStt()}
...
function anim(){if(my.playQ)requestAnimationFrame(anim);if(my.playQ&&!my.ruler.isMovingQ&&!my.pencil.isMovingQ&&!my.compass.isMovingQ&&!my.protractor.isMovingQ){var now=Date.now()
...
return cmd.type;}
function fmtShow(s){return '<div style="text-align:'+my.showAlign+'; height: 90%; margin-top: 4%; width:85%; display: inline-block;">'+s+'</div>';}
...
function doScroll(){my.scrollInc=1.1;for(var i=0;i<2;i++){var div=document.getElementById('show'+i);scrollY=-parseFloat(div.style.top);if(Math.abs(scrollY-my.scrollYTgt)>my.scrollInc){scrollY+=my.scrollInc*Math.sign(my.scrollYTgt-scrollY);div.style.top=-scrollY+'px';}}
return;}
...
function incrFrame(){doScroll();if(my.cmdN>=my.cmds.length){console.log('reached end',pleaseWait(null))
if(!my.strokedQ){g.stroke();my.strokedQ=true;}
...
pencilFade(){if(this.lastDraw<Date.now()-800){if(this.visQ){console.log('pencilFade',this.lastDraw,Date.now())
this.vis(false)}}}
