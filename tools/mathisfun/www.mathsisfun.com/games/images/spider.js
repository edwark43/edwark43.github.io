var w,h,g,my={};function spiderMain(){var version='0.5';w=675;h=500;my.suitRules=[{id:'any',name:'Any'},{id:'alt',name:'Alternating Red Black'},{id:'same',name:'Same'},];my.suitRule=my.suitRules[0]
...
pFr.draw()
pTo.draw()}
function stockDeal(){var OKQ=true
for(var i=0;i<my.tabPiles.length;i++){var p=my.tabPiles[i]
if(p.cards.length==0){OKQ=false
break}}
msg('')
if(!OKQ){msg('Must have at least one card in each pile below','red')
return}
var pFr=my.stockFromPile
for(var i=0;i<my.tabPiles.length;i++){var pTo=my.tabPiles[i]
if(pFr.cards.length<=0)return
var card=pFr.cards[pFr.cards.length-1]
console.log('stockDeal',card)
pFr.sub(card)
pTo.add(card)
pFr.draw()
pTo.draw()
...
stockPassBtnUpdate()}
function stockPassBtnUpdate(){var passLeft=my.stockFromPile.cards.length
...
my.fdnPiles=[]
for(var i=0;i<my.fdnMax;i++){var p=new Pile('fdn','fdn'+i,my.fdnPnt.x+i*my.gap.x,my.fdnPnt.y)
p.draw()
my.piles.push(p)
my.fdnPiles.push(p)}
my.tabPiles=[]
...
while(n<54){for(var i=0;i<my.tabMax;i++){var p=my.tabPiles[i]
var card=my.deck.cards[n];p.add(card)
card.show('back')
n++
if(n>=54)break}}
for(var i=0;i<my.tabMax;i++){var p=my.tabPiles[i]
p.visn=p.cards.length-1
p.draw()}
...
var ps=my.tabPiles.slice()
for(var i=0;i<ps.length;i++){var pFr=ps[i]
var ranks=['K','Q','J','10','9','8','7','6','5','4','3','2','A'];var n=0
var cards=[]
console.log('autoFdn ',pFr.id)
for(var j=0;j<pFr.cards.length;j++){var card=pFr.cards[j]
if(card.face=='front'){console.log('autoFdn ',pFr.id,card.rank,n,ranks[n])
if(card.rank==ranks[n]){if(n==0)cards=[]
cards.push(card)
if(n>=12){autoMove(cards)
return}
n++}else{n==0
cards=[]}}}}
return ''}
...
var pTo=null
for(var i=0;i<my.fdnPiles.length;i++){var p=my.fdnPiles[i]
if(p.cards.length==0){pTo=p
break}}
if(pTo==null)return
for(var i=0;i<cards.length;i++){var card=cards[i]
var pFr=card.pile
console.log('autoMove',card)
pFr.sub(card)
pTo.add(card)
pFr.draw()
pTo.draw()
...
Deck.prototype.loadCards=function(){for(var i=0;i<this.suits.length;i++){for(var j=0;j<this.names.length;j++){this.cards.push(new Card(this.names[j],this.suits[i],j,i,this.backImg,this.frontImg,this.file.rect));this.cards.push(new Card(this.names[j],this.suits[i],j,i,this.backImg,this.frontImg,this.file.rect));}}
gameNew()}
...
Pile.prototype.dragPossible=function(card){switch(this.type){case 'from':return false
...
return n}
Pile.prototype.draw=function(){switch(this.type){case 'fdn':g.strokeStyle='#cdf'
g.lineWidth=2
g.fillStyle='#def'
g.beginPath()
...
g.fill();g.stroke()
break
case 'from':case 'to':case 'free':case 'tab':break
default:}
var addx=0
var addy=0
this.zIndex=10
...
scratchCtx.closePath();scratchCtx.fill();ctx.drawImage(scratchCanvas,0,0);ctx.strokeStyle='#333'
ctx.lineWidth=1
ctx.beginPath()
ctx.roundRect(gap,gap,this.wd-2*gap,this.ht-2*gap,round)
ctx.stroke()}
Card.prototype.drawBack=function(backImg){var g=this.back.getContext('2d');g.strokeStyle='#def';g.fillStyle='#def';g.beginPath();g.roundRect(0,0,this.wd,this.ht,5);g.fill();g.strokeStyle='#777'
g.lineWidth=1
g.beginPath()
var gap=0.6
var round=9.1
