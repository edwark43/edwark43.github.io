var w,h,ratio,i,s,el,g,div,dragQ,game,my={};function fractionaddMain(rel){my.version='0.61';my.rel=typeof rel!=='undefined'?rel:'../';w=440;h=480;my.sectors=[true];my.denom=3;var s='';s+='<div id="main" style="position:relative; width:'+w+'px; background-color: hsl(240,100%,95%); margin:auto; display:block; border: 1px solid black; border-radius: 10px;">';my.typs=['pizza','circle','square'];my.typ=my.typs[0];s+='<div style="position:relative; text-align:right;">';s+=getRadioHTML('','typ',my.typs,'typChg');s+='</div>';var sliceClr='hsl(240,100%,90%)'
my.fracs=[];my.pizzas=[];my.steps=[{title:'Input',fracStrs:[' ','+'],imgStrs:['','+'],inpQ:true,clr:sliceClr},{title:'Each fraction &times; other denominator',fracStrs:['=','+'],imgStrs:['','+'],inpQ:false,clr:sliceClr},{title:'Do the multiplies',fracStrs:['=','+'],imgStrs:['','+'],inpQ:false,clr:sliceClr},{title:'Add',fracStrs:['=','='],imgStrs:['',''],inpQ:false,clr:sliceClr},{title:'Simplify',fracStrs:['=',''],imgStrs:['',''],inpQ:false,clr:sliceClr}];for(var i=0;i<my.steps.length;i++){var step=my.steps[i];s+='<div id="step'+i+'" style="position: relative; width:'+w+'px; text-align:left;float: left; margin-top:5px; background-color:'+step.clr+'; ">';s+='<span style="">'+step.title+'</span>';s+='<br>';s+='<div id="fracs'+i+'" style="display:inline-block; position: relative; width:'+210+'px; vertical-align: middle;">';for(var n=0;n<2;n++){var id='frac'+i+String.fromCharCode(97+n);s+=strHTML(step.fracStrs[n]);s+='<div style="display:inline-block; position: relative; margin:2px; vertical-align:middle;">';s+='<span id="'+id+'"></span>';var frac=new Frac(id,2,3,step.inpQ);my.fracs.push(frac);s+='</div>';}
...
function inputSet(){document.getElementById('frac0anum').value=1;document.getElementById('frac0aden').value=2;document.getElementById('frac0bnum').value=1;document.getElementById('frac0bden').value=3;}
...
my.fracs[6].setMe(cnum+' + '+dnum,dden)
